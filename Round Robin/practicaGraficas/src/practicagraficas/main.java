...
public class main extends javax.swing.JFrame {
    ...
    private int actual_quantum; // Timepo del quantum que se quedo
    ...
    private int quantum = 0; //Quantum para el Round Robin
  ...
  if (ejecucion == null) {
    ...
    boolean cantidadIngresada = true; // Bandera para verificar si ya se ingresó la cantidad de procesos
    do {
                JPanel panel = new JPanel(new GridLayout(2,2));
                JTextField cantidadUI = new JTextField();
                JTextField quantumUI = new JTextField();
                panel.add(new JLabel("Ingrese la cantidad de procesos: \n"));
                panel.add(cantidadUI);
                panel.add(new JLabel("Ingrese el quantum deseado: \n"));
                panel.add(quantumUI);

                int r = JOptionPane.showOptionDialog(null, panel,"Input",JOptionPane.OK_CANCEL_OPTION, JOptionPane.PLAIN_MESSAGE, null, null, null);
                if (r == JOptionPane.OK_OPTION){
                    String cantidadText = cantidadUI.getText();
                    String quantumText = quantumUI.getText();
                    if(cantidadText != null && !cantidadText.isEmpty() && quantumText != null && !quantumText.isEmpty()) {
                    try {
                        ...
                        cantidadIngresada = true; // Marcar que la cantidad fue ingresada correctamente para salir
                        ...
                        JOptionPane.showMessageDialog(null, "Ingrese un número válido.", "Error", JOptionPane.ERROR_MESSAGE);
                        cantidadIngresada = false; // Marcar que la cantidad fue ingresada correctamente para salir
                    }
                }else{
                    break;
                }
 ...
 //Estados de procesos---------------------------------
  ...
  // Mostrar un JOptionPane informando al usuario que no hay procesos en ejecución
    ...
    private void continuar() { //Interrupcion de Continuar o Ejecutar 
     ...
     tiemposProcesos(actual_timer+1, actual_quantum-1); //Vuelve a inicar el proceso en ejecucion en el tiempo donde se quedó
    }
    ...
    private void contadorGlobal() { //Funcion/Hilo del contador global
        contadorThread = new Thread(new Runnable() {
            ...
            public void run() {
                ...
                while (!tiempos_procesos.isEmpty() && ejecutando) { // Mientras la bandera de ejecución sea verdadera
  ...
  private void tiemposProcesos(int time, int quant) { //Funcion/Hilo Tiempos/procesos
    tiemposThread = new Thread(() -> {
        try {
            //boolean ejecutado = false; // Variable para controlar si ya se ejecutó el bloque de código
            int timer = time;
            int quantum_time = quant;
            while (!tiempos_procesos.isEmpty() && tiempos) { // Bucle que se ejecuta mientras tiempos sea verdadero
                actual_timer = timer;
                actual_quantum = quantum_time;
                // Verificar si ejecucion.getTime() es igual a contador_global
                if (ejecucion.getTime() == timer){// && !ejecutado) {
                    ejecucion.setCompletionTime(contador_global-1); //Añade el tiempo de finalizacion
                    int finalizacion = ejecucion.getCompletionTime();
                    int llegada = ejecucion.getTimeArrival();
                    ejecucion.setReturnTime(finalizacion-llegada); //Añade el tiempo de retorno 
                    int retorno = ejecucion.getReturnTime();
